...
export function buildBlock(config: Config): string {
	// Prefer run.sh when the source checkout is available (self-heal rebuilds
	// work); fall back to the compiled binary directly when only the release
	// binary is on disk.
	const entrypoint = config.srcDir ? `${config.srcDir}/run.sh` : config.binPath;
	const prefix = config.providerName !== "github" ? `REVIEW_LOOP_POLLER_PROVIDER=${config.providerName} ` : "";
	const cronLine = `${config.cadence} ${prefix}${entrypoint} >> ${config.logFile} 2>&1`;
